...
MEglmTree <- function(X, Y, part.vars, reg.vars = "1", rand.vars="1",  
                ...
                nAGQ=0, alpha = 0.05, minsize = 10, maxdepth=30, ...){
...
res <- list(tree.fit = tree, glmer.fit = glmer.fit, part.vars=part.vars, groups = groups, 
         ...
         fitted.class = cls, train.perf = perf, threshold = threshold, include.RE=include.RE)
class(res) <- "MEglmTree"         
res         
}
#'
...
MECTree <- function(X, Y, con.tree = FALSE, rhs.vars,  rand.vars="1",  
                ...
                likelihoodCheck = TRUE,glmer.Control=glmerControl(optimizer = "bobyqa"), 
