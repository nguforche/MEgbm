...
#       e$tmp.w <- w 
       tmp.w <<- w
        fit <- glmer(form, data= dat,family = lme.family, 
	              control = glmerControl(optimizer = "bobyqa"), weights = tmp.w,  
...
#       e <- new.env()     
 #      e$tmp.w <- w 
        fit <- lmer(form, data= dat,control = lmerControl(optimizer = "bobyqa"), 
                    weights = tmp.w, verbose = as.numeric(verbose))
	    
	fm = predict(fit, newdata = dat, type = "response")  
	f.hat <- f.hat + 0.5*fm 
